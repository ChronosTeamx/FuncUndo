...
parentPort.on('message', (message: GraphMasterIncoming) => {
  try {
    if (message.type === 'UPDATE_FIREWALL') {
      (globalRegistry as any).validWorkspaceFiles = new Set(message.validFiles);
      (globalRegistry as any).aliases = message.aliases;
    ...
    if (message.type === 'QUERY_BLAST_RADIUS') {
      ...
      const node = (globalRegistry as any).directedGraph.get(uuid);
