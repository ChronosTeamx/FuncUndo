...
async function runOrchestrationLogic(
    ...
    getLatestFunctionHash: (name: string, filePath: string) => Promise<{ hash: string; startLine: number; name: string } | null>,
    findByHash: (hash: string, filePath: string) => Promise<{ name: string; startRow: number } | null>,
...
describe('Orchestrator — Rename Detection', () => {
    ...
    test('does NOT flag as rename when same hash but different row (duplicate body)', async () => {
        ...
        const fn = makeFn({ name: 'funcB', hash: 'hash_abc123', range: { start: { row: 10, column: 0 }, end: { row: 12, column: 1 } } });
...
describe('Orchestrator — Multiple Functions', () => {
    test('only commits changed functions, skips unchanged', async () => {
        ...
        const changed = makeFn({ name: 'changed', hash: 'hash_NEW', range: { start: { row: 5, column: 0 }, end: { row: 8, column: 1 } } });
        const brandNew = makeFn({ name: 'brandNew', hash: 'hash_brandnew', range: { start: { row: 10, column: 0 }, end: { row: 12, column: 1 } } });
